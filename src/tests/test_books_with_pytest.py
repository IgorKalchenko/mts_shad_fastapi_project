...
async def test_get_books(db_session, async_client):
    ...
    assert response.json() == {
        "books": [
            ...
            {"title": "Mziri", "author": "Lermontov", "year": 1997, "id": book_2.id, "count_pages": 104, "seller_id": seller_2.id},
        ]
    }
...
async def test_update_book(db_session, async_client):
    # Создаем книги вручную, а не через ручку, чтобы нам не попасться на ошибку которая
    # может случиться в POST ручке
    seller_3 = sellers.Seller(first_name="Sel2", last_name="Seller2", email="sel2@seller.com", password="selseller2")

    db_session.add_all([seller_3])
    await db_session.flush()

    book = books.Book(author="Pushkin", title="Eugeny Onegin", year=2001, count_pages=104, seller_id=seller_3.id)
    ...
    response = await async_client.put(
        ...
        json={"title": "Mziri", "author": "Lermontov", "count_pages": 100, "year": 2007, "id": book.id, "seller_id": seller_3.id}
    )
    ...
    assert res.seller_id == seller_3.id
